...
final class ActionHandler extends Handler {
	...
	public final void handle(String target, HttpServletRequest request, HttpServletResponse response, boolean[] isHandled) {
		...
		if(target.equals("/")){
			if(devMode){
				System.out.println("###"+new Date()+"## "+ToolWeb.getIpAddr(request)+" send a request of :/ path");
			}			
			renderFactory.getErrorRender(1024).setContext(request, response).render();
			return ;
		}//this is for catch the index paper		
		if (action == null) {
